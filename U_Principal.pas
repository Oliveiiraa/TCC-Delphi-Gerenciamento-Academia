...
begin
  ...
  begin
     ...
     ADOCommand.ConnectionString := ADOConnectionBackup.ConnectionString; //Cria a conexão com o Provider do SQL Server
...
begin
  ...
  begin
    MessageDlg('Você não tem permissão para visualizar estes dados!',mtError,[mbOK],0);
  end;
end;
...
begin
  ...
  if dia = '01' then
  begin
    MessageDlg('Faça o relatório das mensalidades do mês anterior!',mtInformation,[mbOK],0);
...
begin
  ...
  begin
    MessageDlg('Você não tem permissão para consultar funcionário!',mtError,[mbOK],0);
  end;
end;
...
var
    ...
    ano, mes, dia : string;
begin
  ///if Application.MessageBox('Você já realizou o backup hoje?','Atenção',MB_ICONQUESTION + MB_YESNO) = IDNO then
 ///begin
  data_atual := DateToStr(Date);
   ano :=  Copy(data_atual,7,4) ;
   mes :=  Copy(data_atual,4,2) ;
