...
begin
 ...
 // end
  ...
  begin
    MessageDlg('Usuário e/ou senha errado(s)',mtInformation,[mbOK],0);
  end;
...
Text : TStringList;
  i : integer;
begin
  senha := InputBox('Confirmação','Digite o código de acesso','');
  ...
  BEGIN
      ...
      try
        ...
        while not ADOQueryEsqueceSenha.Eof do
          ...
          begin
            Application.MessageBox('Nome do arquivo não informado','Erro',MB_OK + MB_ICONERROR);
            exit;
          end;
  ...
  if senha <> 'BIORITMO' then
    BEGIN
      Application.MessageBox('Código inválido','Erro',MB_OK +MB_ICONERROR);
      exit;
    END;
